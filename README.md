# gittish-core-protocol-repository-list

This package defines the service interface that is responsible for listing the
available repositories for a specific organization.

The service supports the following use cases

* list the repositories for a specific organization based on a specified paging
  directive

The standard course of action for this service is to not filter out any
repositories to which the requesting actor does not have any access to.

In order to filter out any repositories to which the specified actor does
not have any access to, use the repository listing service.
...
```
cd <WORKSPACE_ROOT>/gittish-core-protocol-repository-list
./configure
```
